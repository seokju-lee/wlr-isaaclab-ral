...
class RolloutStorage:
    ...
    def recurrent_mini_batch_generator(self, num_mini_batches, num_epochs=8):
        ...
        padded_advantages_trajectories, _ = split_and_pad_trajectories(self.advantages, self.dones)
        padded_old_mu_trajectories, _ = split_and_pad_trajectories(self.mu, self.dones)
        padded_old_sigma_trajectories, _ = split_and_pad_trajectories(self.sigma, self.dones)
        padded_old_actions_log_prob_trajectories, _ = split_and_pad_trajectories(self.actions_log_prob, self.dones)
        ...
        for ep in range(num_epochs):
            ...
            for i in range(num_mini_batches):
                ...
                yield obs_batch.transpose(0, 1), privileged_obs_batch.transpose(0, 1), actions_batch.transpose(0, 1), values_batch.transpose(0, 1), advantages_batch.transpose(0, 1), returns_batch.transpose(0, 1), old_actions_log_prob_batch.transpose(0, 1), old_mu_batch.transpose(0, 1), old_sigma_batch.transpose(0, 1), (
                    hid_a_batch,
                    hid_c_batch,
                ), masks_batch.transpose(0, 1), rnd_state_batch
